...
def instruction_reward(prompts, completions, ground_truth, ref_model, tokenizer):
    ...
    with torch.no_grad():
        ...
        zero = tokenizer("0")["input_ids"][0]
        one = tokenizer("1")["input_ids"][0]
        select = torch.tensor([zero, one]).cuda()
        tf_logits = logits_interest.index_select(1, select)  # shape: [interest, 2]
    return list(torch.argmax(tf_logits, dim=1).cpu())


def reward_prompt(instruction, response) -> str:
    return f"""\
# Instruction
Repond with "Score: 1" if the response follows the rule.
Repond with "Score: 0" if the response does not follow the rule.
...
def instruction_reward_no_template(
    prompts, completions, ground_truth, ref_model, tokenizer
):
    prompts = [
        reward_prompt(instruction=gt, response=comp[0]["content"])
        for gt, comp in zip(ground_truth, completions)
    ]
    tokenized_prompts = [tokenizer(prompt)["input_ids"] for prompt in prompts]
    input_ids = []
    interest: list = []
    position_ids = []
